...
obc_error_code_t cc1120SendByteReceiveStatus(uint8_t data) {
  ...
  for (uint8_t i = 1; i <= 5; i++) {
    ...
    // Might be better to wait until the MISO pin goes low and then check instead of always implementing a delay,
    // severity depends on how many times this is called
